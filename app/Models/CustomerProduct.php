...
class CustomerProduct extends Model
{
    ...
    public function catProductCount($cat_id){
      $catCount = CustomerProduct::where('category_id',$cat_id)->where('published',1)->where('status',1)->count();
      return $catCount;
    }


    public function stateProductCount($state_id){
      $stateCount = CustomerProduct::where('state_id',$state_id)->where('status',1)->where('published',1)->count();
