<form class="" action="{{route('customer.products.update', $product->id )}}" method="POST" enctype="multipart/form-data" id="choice_form">
    ...
    <div class="card w-md-75 m-auto w-sm-100">
        <h5 class="p-3">Edit Auto Parts & Acessories</h5>
        <div class="card-body">
            <div class="form-group row">
                <div class="col-md-12">
                <label class=" col-from-label font-weight-bold">{{translate('Part or Acessory Type')}} <span class="text-danger">*</span></label>

                    <input type="text" class="form-control" name="name" placeholder="{{ translate('Enter the part or acessory type')}}" value="{{ $product->name }}" required>
                </div>
            </div>
